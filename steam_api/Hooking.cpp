...
// =========== added sme simple cave functions

// general fast write to protected memory
void MEMwrite(void *adr, void *ptr, int size, BYTE *org)
{
	DWORD dwback;
    if (org) memcpy((void *)org,(void *)adr,size);  // only if we want the original bytes
	VirtualProtect(adr,size,PAGE_EXECUTE_READWRITE, &dwback);
	memcpy(adr,ptr,size);
	VirtualProtect(adr,size,dwback, &dwback);
}

// calculate relofs and put it directly in repl (no offset !!)
void MEMreladr(DWORD adr, DWORD cavefunc, PBYTE repl)
{
	*(DWORD *)(repl) = (cavefunc - adr) - 5;			// put the correct offset in the repl string
}

// replace code with naked cave function
void MEMsetcave(DWORD adr, DWORD cavefunc, PBYTE repl, int size, PBYTE org)
{
	MEMreladr(adr,cavefunc,&repl[1]);		// put the correct offset in the repl string
	MEMwrite((void *)adr,(void *)repl,size,org);
}
